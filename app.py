...
FILTER_OPTIONS = [
    ...
    "ESG Score", "CO2 footprint average (kg/kg)",
    "Embodied energy average (MJ/kg)", "Water usage average (l/kg)",
    "Toxicity", "Max Companionability",
]
CRITERIA_OPTIONS = {
    "Reserve (ton)": 1,  "Production (ton)": 1,  "HHI (USGS)": -1,
    "ESG Score": -1,     "CO2 footprint average (kg/kg)": -1,
    "Embodied energy average (MJ/kg)": -1, "Water usage average (l/kg)": -1,
...
# ─────────────────────────────────────────────────────────────────────────────
DF1 = pd.read_excel("7_web_database.xlsx")

# OPT: pre-clean Element columns in DF1 once
for _c in ELEMENT_COLUMNS:
    if _c in DF1.columns:
        DF1[_c] = DF1[_c].fillna("").astype(str).str.strip()

# OPT: pre-convert Bandgap to numeric once
if "Bandgap" in DF1.columns:
    DF1["Bandgap"] = pd.to_numeric(DF1["Bandgap"], errors="coerce")

# Derive Elsevier publication subset — used by bandgap temporal scatter
DF2 = DF1[
...
def entropy_weights(matrix: np.ndarray) -> np.ndarray:
    ...
    div_sum     = diversities.sum()
    return diversities / div_sum if div_sum > 1e-10 else np.ones(m_cols) / m_cols
...
INTEGER_FILTERS    = {"Max Companionability", "Toxicity"}
...
def bg_update_all(included, seed):
    ...
    if df2_grp.empty or "Bandgap" not in df2_grp.columns:
        fig_t    = empty_fig
        ...
        df2_grp  = df2_grp.dropna(subset=["Date", "Bandgap", "Frequency", "Name"])
        df2_plot = df2_grp.sort_values("Date")

        vmax  = float(df2_plot["Bandgap"].max()) if not df2_plot.empty else 4.0
        fig_t = px.scatter(
            df2_plot, x="Date", y="Bandgap", color="Name",
            size="Frequency", size_max=30,
            labels={"Bandgap": "Bandgap Energy (eV)", "Date": "Publication Date"},
        ...
        # Sample table — Name, Bandgap, DOI, Date (MM/YYYY)
        n = min(10, len(df2_doi))
        if n == 0:
            table_el = html.P("No records to display.")
        else:
            sample = df2_doi.sample(n=n, random_state=seed).reset_index(drop=True).copy()
            sample["Date"] = pd.to_datetime(sample["Date"], errors="coerce").dt.strftime("%m/%Y")
...
LONG_TERM_HIGH  = {"ESG Score", "Toxicity", "Max Companionability", "Reserve (ton)"}
SHORT_TERM_HIGH = {"Production (ton)", "HHI (USGS)", "CO2 footprint average (kg/kg)",
                   "Water usage average (l/kg)", "Embodied energy average (MJ/kg)"}
...
def show_weights_table(weighting, filters, excluded, *raw_w):
    ...
    if weighting == "Manual":
        ...
        wdf = pd.DataFrame({
            "Criterion": crit_cols,
            "Weight (%)": [round(w * 100, 2) for w in w_norm],
            "Direction": ["Maximize" if avail[c] == 1 else "Minimize" for c in crit_cols],
        })
    ...
    # ── Entropy: compute weights from filtered matrix and show table ──────────
    nan_rows  = df_f[crit_cols].isna().any(axis=1)
    n_dropped = int(nan_rows.sum())
    df_clean  = df_f[~nan_rows]

    if df_clean.empty:
        return dbc.Alert("⚠️ No materials remain after removing N/A rows.", color="warning")

    matrix = df_clean[crit_cols].values.astype(float)

    if np.isnan(matrix).any() or np.any(matrix < 0):
        return dbc.Alert("⚠️ Cannot compute entropy weights: matrix contains missing or negative values.", color="warning")

    try:
        w_norm = entropy_weights(matrix)
    except Exception as e:
        return dbc.Alert(f"⚠️ Entropy calculation failed: {e}", color="warning")
    ...
    note = (dbc.Alert(f"ℹ️ {n_dropped} material(s) with N/A excluded before weight calculation.",
                      color="warning", className="py-2 mt-1")
            if n_dropped > 0 else html.Div())
    return html.Div([
        html.H6("Entropy Criteria Weights"),
        note,
        dbc.Table.from_dataframe(wdf, striped=True, bordered=True,
                                 hover=True, responsive=True, className="small"),
    ])
